...
# --- MODIFICATION: Added PENDING_APPROVAL and REJECTED statuses for the new workflow ---
class MessageStatusEnum(str, enum.Enum):
    PENDING_APPROVAL = "pending_approval" # New status for items in the Approval Queue
    REJECTED = "rejected" # New status for items rejected from the queue
    ...
    PROCESSING_SEND = "processing_send"
    APPROVED = "approved"
...
class NudgeTypeEnum(str, enum.Enum):
    ...
    POTENTIAL_TARGETED_EVENT = "potential_targeted_event"
    STRATEGIC_ENGAGEMENT_OPPORTUNITY = "strategic_engagement_opportunity"
...
class CoPilotNudge(Base):
    ...
    __table_args__ = (Index('idx_copilotnudge_business_type_status', 'business_id', 'nudge_type', 'status'),)
    def __repr__(self):
...
class TargetedEvent(Base):
    ...
    __table_args__ = (Index('idx_targetevent_biz_cust_dt', 'business_id', 'customer_id', 'event_datetime_utc'),)
    def __repr__(self):

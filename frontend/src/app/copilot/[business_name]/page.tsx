...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE || process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:8000';
...
const CoPilotPage: FC<CoPilotPageProps> = ({ params: paramsProp }) => {
  ...
  const handleActivatePlan = useCallback(async (nudgeId: number, customerId: number, finalMessages: any[]) => {
    ...
    try {
      ...
      const nudge = actionableNudges.find(n => n.id === nudgeId);
  ...
  }, [setIsActionLoading, setAllActiveNudges, addNotification, actionableNudges, router, businessSlug]);
  ...
  }, [setIsActionLoading, setAllActiveNudges, addNotification, actionableNudges]);
  ...
  const handleLaunchGrowthCampaign = useCallback(async (nudgeId: number) => {
    ...
    try {
      ...
      addNotification({ type: 'success', title: 'Drafts Created!', message: `${resultData.drafts_created || 0} drafts are ready for your review. Redirecting...` });
      setAllActiveNudges(prev => prev.filter(n => n.id !== nudgeId));
  ...
  return (
    <section>
      <h2 className="text-3xl font-semibold mb-5 text-slate-100 flex items-center">
        <SparklesIcon className="w-8 h-8 mr-3 text-yellow-400 opacity-90" /> Action Center
        {nudges.length > 0 && <span className="ml-4 bg-yellow-500/20 text-yellow-300 text-sm font-semibold px-3 py-1 rounded-full">{nudges.length}</span>}
      </h2>
      ...
      {!isLoading && nudges.length === 0 && (
        <div className="text-center py-12 bg-slate-800/50 rounded-lg border border-slate-700/60">
          ...
          <p className="text-slate-200 text-lg font-semibold">No immediate actions needed!</p><p className="text-slate-400">Co-Pilot is monitoring for new opportunities.</p>
        </div>
      )}
              ...
              <div className="p-5">
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    <div className="min-w-0"><p className="text-xs uppercase tracking-wider font-semibold text-purple-300">{isEvent ? "Potential Appointment" : "Follow-up Plan"}</p><h3 className="text-2xl font-bold text-slate-50 truncate">{nudge.customer_name}</h3></div>
                  </div>
                ...
                <div className="mt-5 flex flex-col sm:flex-row items-center justify-between gap-4 p-4 bg-slate-900/50 rounded-lg">
                  ...
                  <button onClick={() => handleToggleActionItemExpansion(nudge.id)} className="flex-shrink-0 w-full sm:w-auto bg-gradient-to-r from-purple-500 to-pink-500 text-white font-semibold px-6 py-3 rounded-lg shadow-lg hover:scale-105 transition-transform duration-200 flex items-center justify-center">{isEvent ? "Confirm Appointment" : "Review Plan"}<ArrowRight className="w-5 h-5 ml-2" /></button>
                </div>
              </div>
              {isExpanded && (
                <div className="bg-slate-900/70 p-4 sm:p-6 border-t border-slate-700/50">
                  {isEvent ? (<PotentialTimedCommitmentCard nudge={nudge} onConfirm={handleConfirmEvent} onDismiss={() => handleDismiss(nudge.id)} onViewConversation={handleViewConversation} />) : (<EngagementPlanCard nudge={nudge} onActivatePlan={handleActivatePlan} onDismiss={() => handleDismiss(nudge.id)} onViewConversation={handleViewConversation} />)}
                </div>
              )}
            </div>
          );
...
const SentimentSection: FC<SentimentSectionProps> = ({ groupedNudges, setAllActiveNudges, setIsActionLoading, addNotification, businessSlug }) => { 
  ...
  return (
    <section>
      <h2 className="text-2xl font-semibold mb-4 text-slate-100 border-b-2 border-slate-700 pb-2 flex items-center">Customer Sentiment</h2>
  ...
  return (
    <section>
      <h2 className="text-2xl font-semibold mb-4 text-slate-100 border-b-2 border-slate-700 pb-2 flex items-center">
        <TrendingUpIcon className="w-7 h-7 mr-3 text-purple-400" /> Unlock Growth
        {nudges.length > 0 && <span className="ml-4 bg-purple-500/20 text-purple-300 text-sm font-semibold px-3 py-1 rounded-full">{nudges.length}</span>}
      </h2>
      <div className="space-y-4">
        ...
        {!isLoading && nudges.length === 0 && (
          <div className="text-center py-8 px-4 bg-slate-800/40 rounded-lg border border-slate-700/60">
            ...
            <p className="text-slate-300 font-medium">No new growth strategies</p><p className="text-slate-400 text-sm">Co-Pilot is analyzing your data for opportunities.</p>
          </div>
        )}
        {nudges.map(nudge => (<GrowthOpportunityCard key={nudge.id} nudge={nudge} onLaunchCampaign={handleLaunchGrowthCampaign} isLoading={isActionLoading === nudge.id} />))}
      </div>
    </section>
  );
};

...
export default function ContactEngagementPage() {
  ...
  const handleApprove = async (msgId: number) => {
    try {
      await apiClient.put(`/review/${msgId}/schedule`);
      setMessages(prev => prev.map(msg => msg.id === msgId ? { ...msg, status: "scheduled" } : msg));
    } catch (err) {
  ...
  const handleDelete = async (msgId: number) => {
    try {
      await apiClient.delete(`/review/${msgId}?source=roadmap`);
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-4 bg-nudge-gradient text-white min-h-screen">
      ...
      {loading ? (
        ...
        <p className="text-neutral italic">
          No messages planned for this contact. The roadmap may not have generated successfully.
        </p>
      ) : (
        <div className="relative border-l-4 border-purple-500 ml-8 mt-12">
          {messages.map((msg, idx) => {
            ...
            const month = dateObj ? format(dateObj, "LLL").toUpperCase() : "";
            ...
            return (
              <div key={msg.id} className="relative mb-12 pl-10">
                {/* Timeline badge */}
                <div className="absolute -left-7 top-1/2 transform -translate-y-1/2 flex flex-col items-center gap-1">
                  ...
                  <div className="w-px flex-1 bg-purple-500"></div>
                </div>

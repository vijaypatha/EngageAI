// frontend/src/app/InstantNudgePage.tsx 
"use client";
...
import { Loader2, Info, Trash2, MessageSquarePlus, Settings2, CalendarClock, Send, CheckCircle2, Clock3, Filter, UserX, Eye } from "lucide-react"; // Added Filter, UsersX, Eye
...
// Helper function (getLocalDateForInput - kept from original if still used, though datetime-local binding is direct)
...
const LIFECYCLE_STAGES_FILTER_OPTIONS = ["New Lead", "Active Client", "Past Client", "Prospect", "VIP"]; // For filtering
...
export default function InstantNudgePage() {
  ...
  const [selectedLifecycleStages, setSelectedLifecycleStages] = useState<string[]>([]); // New state
  ...
  useEffect(() => {
    ...
    if (!slugParam) {
        ...
        setIsLoadingContacts(false); setIsLoadingTags(false); return;
    }
    setIsLoadingContacts(true); setIsLoadingTags(true); setError(null);
    apiClient
      ...
      .catch((err) => {
        ...
        setIsLoadingContacts(false); setIsLoadingTags(false);
      });
  }, [businessSlug]);
    ...
    const fetchData = async () => {
      ...
      try {
        const [customersData, tagsData] = await Promise.all([
          getCustomersByBusiness(businessId), getBusinessTags(businessId)
        ]);
        if (!isMounted) return;
        ...
        if (isMounted) { setIsLoadingContacts(false); setIsLoadingTags(false); }
      }
    };
    fetchData();
  ...
  const filteredContacts = useMemo(() => {
    let contacts = allOptedInContacts;
    ...
    // New lifecycle stage filter logic
    if (selectedLifecycleStages.length > 0) {
        contacts = contacts.filter(customer =>
            customer.lifecycle_stage && selectedLifecycleStages.includes(customer.lifecycle_stage)
        );
    }
    return contacts;
  }, [allOptedInContacts, selectedFilterTags, selectedLifecycleStages]); // Added selectedLifecycleStages

  const updateNudgeBlock = (index: number, field: keyof NudgeBlock, value: any) => {
     setNudgeBlocks(prev => {
       const copy = [...prev]; const block = { ...copy[index] }; (block as any)[field] = value;
       if (['topic', 'message', 'customerIds', 'schedule', 'datetime'].includes(field as string)) {
           block.isDrafting = false; block.isSending = false; block.isScheduled = false; block.isSent = false; block.error = null;
       }
       copy[index] = block; return copy;
     });
   };
  ...
  const handleDraft = async (index: number) => {
    ...
    try {
      // Pass filter context if backend supports it
      const draftContextPayload: any = {};
      if (selectedFilterTags.length > 0) draftContextPayload.filter_tags = selectedFilterTags.map(t => t.id);
      if (selectedLifecycleStages.length > 0) draftContextPayload.filter_lifecycle_stages = selectedLifecycleStages;

      const res = await apiClient.post<{ message_draft?: string }>("/instant-nudge/generate-targeted-draft", {
        topic: block.topic, business_id: businessId, customer_ids: block.customerIds, ...draftContextPayload
      });
  ...
  const handleSendOrSchedule = async (index: number) => {
    const block = nudgeBlocks[index];
    if (!businessId || !block.message || block.customerIds.length === 0) { updateNudgeBlock(index, 'error', 'Message and at least one Recipient are required.'); return; }
    if (block.schedule && !block.datetime) { updateNudgeBlock(index, 'error', 'Please select a date and time for scheduling.'); return; }
    updateNudgeBlock(index, 'isSending', true); updateNudgeBlock(index, 'error', null);
    const payload = { customer_ids: block.customerIds, message: block.message, business_id: businessId, send_datetime_utc: block.schedule && block.datetime ? new Date(block.datetime).toISOString() : null };
    try {
      ...
      if (details) {
        ...
        if (block.schedule) {
          ...
          else if (details.scheduled_count > 0) { updateNudgeBlock(index, 'isScheduled', true); updateNudgeBlock(index, 'error', `Scheduled for ${details.scheduled_count}/${block.customerIds.length}. ${details.failed_count > 0 ? `${details.failed_count} failed.` : ''}`); }
          ...
          else if (details.sent_count > 0) { updateNudgeBlock(index, 'isSent', true); updateNudgeBlock(index, 'error', `Sent to ${details.sent_count}/${block.customerIds.length}. ${details.failed_count > 0 ? `${details.failed_count} failed.` : ''}`); }
   ...
   const clearTagFilters = () => setSelectedFilterTags([]);

   // New handlers for Lifecycle Stage Filter
   const handleLifecycleStageToggle = (stage: string) => {
     setSelectedLifecycleStages(prev =>
         prev.includes(stage) ? prev.filter(s => s !== stage) : [...prev, stage]
     );
   };
   const clearLifecycleFilters = () => setSelectedLifecycleStages([]);
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-12"> {/* Increased max-width to 6xl for more space */}
       ...
       <div className="bg-sky-800/20 border border-sky-700/40 rounded-xl p-4 mb-10 text-sm text-sky-200 shadow-lg"> {/* Increased mb */}
         <p className="flex items-center"><Info className="h-5 w-5 mr-3 flex-shrink-0 text-sky-400" />
            <span>Only opted-in contacts are available for selection ({allOptedInContacts.length} total). Filters below apply to this list.</span>
         </p>
       </div>

      {/* --- Filter Section --- */}
      <div className="mb-12 p-6 bg-slate-800/70 rounded-xl border border-slate-700/80 shadow-xl space-y-6"> {/* Increased padding and mb */}
        <div className="flex items-center text-xl font-semibold text-slate-100 mb-2">
            <Filter size={24} className="mr-3 text-purple-400"/>
        ...
        <div>
            ...
            {isLoadingTags ? <Loader2 className="h-5 w-5 animate-spin text-purple-400" /> : availableTags.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                    {availableTags.map(tag => {
                        ...
                        return (<Button key={tag.id} variant="outline" size="sm" onClick={() => handleFilterTagToggle(tag)}
            ...
            {selectedFilterTags.length > 0 && (<Button variant="link" size="sm" className="text-xs text-purple-400 hover:text-purple-300 mt-2.5 p-0 h-auto" onClick={clearTagFilters}>Clear Tag Filters</Button> )}
        </div>

        {/* NEW: Lifecycle Stage Filter */}
        <div>
            ...
            {/* Assuming LIFECYCLE_STAGES_FILTER_OPTIONS is available and populated */}
            {LIFECYCLE_STAGES_FILTER_OPTIONS.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                    {LIFECYCLE_STAGES_FILTER_OPTIONS.map(stage => {
                        const isSelected = selectedLifecycleStages.includes(stage);
                        return (<Button key={stage} variant="outline" size="sm" onClick={() => handleLifecycleStageToggle(stage)}
            ...
            {selectedLifecycleStages.length > 0 && (<Button variant="link" size="sm" className="text-xs text-purple-400 hover:text-purple-300 mt-2.5 p-0 h-auto" onClick={clearLifecycleFilters}>Clear Lifecycle Filters</Button> )}
        </div>
      </div>
      ...
      {nudgeBlocks.map((block, index) => {
        ...
        if (block.customerIds.length > 0) {
            sampleCustomerForPreview = allOptedInContacts.find(c => c.id === block.customerIds[0]);
        } else if (filteredContacts.length > 0) {
            sampleCustomerForPreview = filteredContacts[0];
        }
        ...
        return (
            <div key={block.id} className={cn("p-6 rounded-xl mb-10 border shadow-xl transition-all duration-300 relative backdrop-blur-sm", block.isSent ? "bg-green-800/30 border-green-700/50" : block.isScheduled ? "bg-sky-800/30 border-sky-700/50" : "bg-slate-800/60 border-slate-700/70 hover:border-purple-500/50 hover:shadow-purple-500/10")}> {/* Increased padding and mb */}
                <h2 className="text-xl font-semibold text-slate-100 mb-6 border-b border-slate-700 pb-4 flex justify-between items-center"> {/* Increased mb and pb */}
                    <span>Message Block #{index + 1}</span>
                    {nudgeBlocks.length > 1 && !block.isSent && !block.isScheduled && (
                        <Button variant="ghost" size="icon" className="text-slate-400 hover:text-red-400 hover:bg-red-700/20 rounded-lg h-9 w-9" onClick={() => removeNudgeBlock(index)} title="Remove this message block"> {/* Adjusted hover and rounded */}
                            <Trash2 size={18}/>
                        </Button>
                    )}
                </h2>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6"> {/* Increased gaps */}
                    {/* Left Column: Customer Selection & Topic */}
                    <div className="space-y-6"> {/* Increased spacing */}
                        <div>
                            <Label className="text-base font-medium text-slate-200 block mb-2">Target Customers <span className="text-sm text-slate-400">({filteredContacts.length} matching filters)</span></Label>
                            <div className="bg-slate-700/60 rounded-lg p-3.5 border border-slate-600/70 max-h-52 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-500 scrollbar-track-slate-700"> {/* Increased padding and max-h */}
                                {isLoadingContacts ? <div className="flex justify-center items-center h-20"><Loader2 className="h-6 w-6 animate-spin text-purple-400" /></div> :
                                 filteredContacts.length > 0 ? (
                                    <>
                                        <label className="flex items-center text-slate-100 mb-2.5 font-medium cursor-pointer hover:bg-slate-600/60 p-2 rounded-md transition-colors"> {/* Adjusted padding */}
                                        ...
                                        <label key={c.id} className="flex items-center text-slate-200 mb-1.5 cursor-pointer hover:bg-slate-600/60 p-2 rounded-md transition-colors text-sm"> {/* Adjusted padding */}
                                            <input type="checkbox" className="mr-3 h-4 w-4 accent-purple-500 bg-slate-800 border-slate-500 rounded focus:ring-purple-500 focus:ring-offset-slate-700"
                                ...
                                ) : ( /* ENHANCED: No Matching Contacts State */
                                    <div className="text-center py-8 text-slate-400 flex flex-col items-center justify-center h-full">
                                        ...
                                        <p className="text-xs mt-1">Try adjusting your Tag or Lifecycle Stage selections above.</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        <div>
                            ...
                            <Input id={`topic-input-${index}`} placeholder="e.g., Holiday Special, Appointment Reminder"
                                className="bg-slate-700 border-slate-600 text-slate-100 placeholder:text-slate-400 focus:ring-1 focus:ring-purple-500 focus:border-purple-500 rounded-md shadow-sm py-2.5 px-3" /* Adjusted padding */
                        ...
                        <Button variant="outline" className="w-full py-2.5 border-sky-500/70 bg-sky-600/20 hover:bg-sky-600/40 text-sky-200 hover:text-sky-100 rounded-md shadow-md transition-colors group text-sm font-semibold" /* Adjusted padding and style */
                            onClick={() => handleDraft(index)} disabled={!block.topic || !businessId || block.isSent || block.isScheduled || block.isSending || block.isDrafting}>
                    ...
                    <div className="space-y-6"> {/* Increased spacing */}
                        <div>
                            ...
                            <Textarea id={`message-textarea-${index}`} placeholder="AI draft will appear here, or write your own. Use {customer_name} for personalization."
                                className="bg-slate-700 border-slate-600 text-slate-100 placeholder:text-slate-400 focus:ring-1 focus:ring-purple-500 focus:border-purple-500 rounded-md shadow-sm min-h-[140px] p-3 scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-slate-700/50" /* Increased min-h and padding */
                            ...
                            {/* NEW: Character and SMS Segment Count */}
                            <p className="text-xs text-slate-400 mt-2 text-right">
                                Chars: {charCount} | Segments: {segmentCount}
                            </p>
                        </div>

                        {/* NEW: Live Personalization Preview */}
                        {(block.message.includes("{customer_name}") || block.message.includes("{{customer_name}}")) && ( // Check for both common placeholder types
                            <div>
                                ...
                                <div className="bg-slate-700/40 p-3 rounded-md text-slate-200 text-sm border border-slate-600/50 min-h-[50px] whitespace-pre-wrap shadow-inner">
                        ...
                        <div>
                            <Label className="text-base font-medium text-slate-200 block mb-2">Send Options</Label>
                            <div className="flex flex-col sm:flex-row sm:items-center gap-x-6 gap-y-3 p-3.5 bg-slate-700/60 border border-slate-600/70 rounded-lg shadow-sm"> {/* Increased padding */}
                                <Label className="text-slate-100 flex items-center gap-2.5 cursor-pointer"> {/* Increased gap */}
                                ...
                                {block.schedule && (
                                    <Input type="datetime-local" className="bg-slate-700 border-slate-600 text-slate-100 placeholder:text-slate-400 focus:ring-1 focus:ring-purple-500 focus:border-purple-500 rounded-md shadow-sm p-2 text-sm h-9 sm:ml-auto" value={block.datetime} onChange={e => updateNudgeBlock(index, 'datetime', e.target.value)} min={new Date(Date.now() + 60000).toISOString().slice(0, 16)} disabled={block.isSent || block.isScheduled || block.isSending} required={block.schedule} /> /* Adjusted padding */
                                )}
                            </div>
                        </div>
                    </div>
                ...
                {/* Actions and Status */}
                <div className="flex flex-col sm:flex-row justify-end items-center gap-x-4 gap-y-2 border-t border-slate-700/80 pt-5 mt-8"> {/* Adjusted spacing and border */}
      ...
      <div className="mt-12 text-center"> {/* Increased mt */}

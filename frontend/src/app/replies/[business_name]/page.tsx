...
export default function RepliesPage() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (id) {
          ...
          // - Customer has sent a message (reply.response exists)
          // - The engagement status is 'pending_review' (meaning AI might have a draft, or it needs a new reply)
  ...
  const handleSend = async (replyId: number) => {
    try {
      ...
      // Use edited content if available, otherwise use the existing ai_response (AI draft)
      ...
      // The endpoint expects `content` not `updated_content` based on previous discussions,
      // but the backend route /engagement-workflow/reply/{id}/send uses `updated_content`. Let's align.
      // It seems your backend engagement_workflow_routes.py for `PUT /reply/{id}/send`
      // expects `payload: SendDraftInput` which has `updated_content: str`.
      ...
      setTimeout(() => {
        ...
        setSentIds((prev) => prev.filter((id) => id !== replyId)); // Clean up sentIds too
      }, 800); // UI removal delay
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-nudge-gradient text-white px-6 py-12">
      ...
      <div className="space-y-6">
        {replies.map((reply) => (
          <React.Fragment key={reply.id}> {/* Key moved to Fragment */}
            <div
            ...
            >
              ...
              {/* Assuming 'phone' might come from a joined customer object if the endpoint provides it */}
              ...
              {reply.timestamp && ( // This timestamp should be for when the customer's message (reply.response) was received/sent
              ...
              <div className="mt-4 flex flex-wrap justify-between items-center gap-2">
                {editingId === reply.id ? (
                  <div className="flex items-center gap-3">
                        ...
                        if (editedContent && editedContent !== (reply.ai_response ?? "")) {
                          try {
                            await apiClient.put(`/engagement-workflow/reply/${reply.id}/edit`, {
                              ai_response: editedContent, // Backend for edit expects 'ai_response'
                            });
                            setReplies((prev) =>
                              prev.map((r) =>
                    ...
                    <button
                      ...
                      onClick={() => {
                        // Reset editedReplies for this id to the original ai_response
                  ...
                  <button
                    ...
                    onClick={() => {
                      // Ensure editedReplies has the current ai_response when starting to edit
                      setEditedReplies(prev => ({...prev, [reply.id]: reply.ai_response ?? ""}));
                ...
                <button
                  className={`px-4 py-2 rounded text-white font-medium transition ${
                    // Determine if the button should be active or disabled-looking
                    // Disabled looking if:
                    // 1. Currently editing: and the edited text is empty or same as original AI draft
                    // 2. Not editing: and there's no AI draft to send
                  ...
                  disabled={
                    // Actually disable if:
                    // 1. Currently editing: and edited text is empty
                    // 2. Not editing: and AI draft is empty
                    editingId === reply.id

...
import { Loader2, Info, UserPlus, AlertTriangle, Zap, MessageSquareText, Tags, Lightbulb, CheckCircle } from "lucide-react";
...
export default function AddContactPage() {
  ...
  const appendToTextarea = (fieldName: "pain_points" | "interaction_history", text: string) => {
    setForm(prev => ({
        ...prev,
        [fieldName]: prev[fieldName] + (prev[fieldName].trim() ? "\n- " : "- ") + text + (text.endsWith("]") ? "" : " ") // Add space if not ending with placeholder
    }));
    // Focus the textarea after appending might be a good UX, but optional
  ...
  const handleSubmit = async () => {
    ...
    if (error && (!businessId || !businessProfile)) {
